...
from utils.utils import current_milli_time, ensure_exists, make_power_2_cv2
...
log = logging.getLogger('FP.processor')
...
class FormProcessor:
    def __init__(self, work_dir:str = '/tmp/form-segmentation', config:object=None, cuda: bool = False) -> None:
        log.info("Form processor deux [cuda={}]".format(cuda))
    ...
    def __load(self):
        ...
        self.overlay_processor = FormOverlay(work_dir=work_dir, cuda=self.cuda)
    ...
    def process(self, img_path):
        m0 = current_milli_time()
        work_dir =  self.work_dir
        docId = img_path.split('/')[-1].split('.')[0] # docId
        log.info('[%s] Processing image : %s', docId, img_path)
        debug_dir = ensure_exists(os.path.join(work_dir, docId, 'work'))
        dataroot_dir = ensure_exists(os.path.join(self.work_dir, docId, 'dataroot'))
        ...
        for field_config in fields:
            ...
            try:
                ...
                if heuristics['enabled']:
                    heuristics_applied, heuristics_snippet = self.apply_heuristics(docId, field, box, overlay_img, heuristics)    
                    m1 = current_milli_time()
                    log.info('[%s] [%s] heuristics applied, time : %s, %s(ms)', docId, field, heuristics_applied, m1-m0)
                ...
                if heuristics_applied:
                    ...
                    imwrite(os.path.join(fields_aggro_dir, '%s.png' % (docId)), snippet)
                    imwrite(os.path.join(fields_clean_aggro_dir, '%s.png' % (docId)), heuristics_snippet)
                ...
                m0 = current_milli_time()
                boxes, fragments, lines, _ = box_processor.extract_bounding_boxes(docId, field, snippet)
                m1 = current_milli_time()
                log.info('[%s] [%s] Box processor time : %s(ms)', docId, field, m1 - m0)

                m0 = current_milli_time()
                icr_results, icr_overlay = icr_processor.recognize(docId, field, snippet, boxes, fragments, lines)

                debug_dir = ensure_exists(os.path.join(work_dir, docId, 'work_figures'))
                ...
                m1 = current_milli_time()
                log.info('[%s] [%s] ICR processor time : %s(ms)', docId, field, m1 - m0)
                log.info('[%s] [%s] Field eval time : %s(ms)', docId, field, m1 - s)
            except Exception as ident:
                failed = True
        ...
        log.info('[%s] [] Eval time : %s(ms)', docId, e-s)
        return {
            'meta': {'imageSize': {'width': img.shape[1], 'height': img.shape[0]}, 'id':docId},
            'eval_time':m1,
